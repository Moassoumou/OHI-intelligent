...
function redirectToDashboard(email) {
  localStorage.setItem('ohi_user_email', email);
...
function loginWithSSO(provider) {
  ...
  setTimeout(() => {
    hideLoadingModal();
    const email = `utilisateur@${provider}.com`;
...
function testLogin() {
  ...
  setTimeout(() => {
    hideLoadingModal();
    redirectToDashboard('demo@entreprise.com');
  }, 800);
}
...
function logout() {
  localStorage.removeItem('ohi_user_email');
